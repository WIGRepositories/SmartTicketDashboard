...
var ctrl = app.controller('myCtrl', function ($scope, $http, $localStorage, $uibModal, fileReader, $upload, $filter) {
    ...
    $scope.Getbranddetails = function () {

        $scope.Dl = null;

        $scope.selectedlistusers = parseLocation(window.location.search)['Id'];

        $http.get('/api/BrandAmbassador/Getbranddetails?Id=' + $scope.selectedlistusers).then(function (res, data) {
            ...
            for (i = 0; i < $scope.initdata.Table.length; i++) {
                if ($scope.initdata.Table[i].Id == $scope.Dl.Gender) {
                    $scope.Dl.gender = $scope.initdata.Table[i];
                    break;
                }
            }

            for (i = 0; i < $scope.initdata.Table1.length; i++) {
                if ($scope.initdata.Table1[i].Id == $scope.Dl.CountryId) {
                    $scope.Dl.Country = $scope.initdata.Table1[i];
                    break;
                }
            }
